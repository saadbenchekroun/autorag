...
class IndexingPipelineService:
        ...
        if model_choice not in self.embeddings_cache:
            if "openai" in model_choice:
                ...
                if api_key:
                    ...
                    model_name = "text-embedding-3-small" if "small" in model_choice else "text-embedding-3-large"
                    self.embeddings_cache[model_choice] = OpenAIEmbeddings(model=model_name)
                    return self.embeddings_cache[model_choice]
                else:
                    print(f"Warning: OPENAI_API_KEY not found. Falling back to local open-source BGE model instead of {model_choice}.")
            ...
            if "minilm" in model_choice or model_choice == "huggingface_minilm":
                ...
                print(f"Note: Mapping {model_choice} to optimized BGE local model for rapid testing.")
    ...
    def execute_pipeline(self, project_id: str, architecture_decision: Dict[str, Any], documents: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        if chunking_strategy == "fixed_size":
            splitter = CharacterTextSplitter(separator="", chunk_size=chunk_size, chunk_overlap=overlap_size)
        ...
        if db_choice != "chroma":
            ...
            print(f"[Simulation] Note: Falling back to local ChromaDB payload for demo persistence.")

...
class RAGRuntimeSystem:
    ...
    def generate_response(self, project_id: str, query_text: str, architecture_decision: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        persist_dir = os.path.join(os.getcwd(), config.get_nested("pipeline.ingestion.directories.chroma_db_dir", "chroma_db"), project_id)
        if not os.path.exists(persist_dir):
            return {"error": "Vector database not found for this project. Please index documents first."}
        ...
        try:
            ...
            context_used = [{"text": doc.page_content[:200] + "...", "source": doc.metadata.get("source", "unknown")} for doc in docs]
        ...
        api_key = os.getenv("OPENAI_API_KEY")
        if not api_key:
            answer = "API Key not found. Displaying retrieved context answering your query:\n\n" + context_text
        else:
